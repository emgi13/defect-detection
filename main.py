...
def loadImage(path: str, gray=False) -> np.ndarray:
    LOG.info(f"Loading : {path} with gray={gray}")
...
def main():
    ...
    with logging_redirect_tqdm():
        # Get all files
        for vid_no in trange(1, 7, desc="Videos", position=1):
            files = glob(f"./raw/frames/src_{vid_no}_*.png")
            files.sort()
            rad = 0
            clip_min = 0
            clip_max = 0
            for file in tqdm(files, desc="Frames", position=0):
                # if rad != 0:
                #     return
                ...
                if rad == 0:
                    ...
                    LOG.info(f"Set radius to {rad:5.2f}px")
                ...
                LOG.info(f"Found {len(yInd):6d} centers")
                ...
                plt.figure(figsize=(img.shape[1] / 300, img.shape[0] / 300))
                plt.imshow(
                    grid_z, cmap="plasma", vmin=0.0, vmax=7.0, interpolation="none"
                )
                plt.axis("off")
                plt.savefig(
                ...
                if clip_min == 0 and clip_max == 0:
                    ...
                    clip_min = avg - 7 * std
                    clip_max = avg + 7 * std
                    LOG.info(f"Setting clipping to {clip_min:5.3f}:{clip_max:5.2f}")
